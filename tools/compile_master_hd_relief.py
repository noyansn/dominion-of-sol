import os
import sys
import numpy as np
from PIL import Image

def compile_natural_earth_master_hd(
    width=4096, height=2048,
    landcover_tif='tools/data/NE2_50M_SR_W/NE2_50M_SR_W/NE2_50M_SR_W.tif',
    bathy_tif='tools/data/OB_50M/OB_50M/OB_50M.tif',
    mask_path='client/src/assets/world_visual_mask.bin',
    output_relief_rgba='client/src/assets/world_relief.rgba',
    output_relief_png='client/src/assets/world_relief.png',
    output_bathy_rgba='client/src/assets/world_bathymetry.rgba',
    output_bathy_png='client/src/assets/world_bathymetry.png'
):
    print(f"=== [ART-03] MASTER NATURAL EARTH II COMPILATION ({width}x{height}) ===")
    Image.MAX_IMAGE_PIXELS = None

    # 1. Load Visual Land Mask (4096x2048)
    print("Loading authoritative land mask...")
    raw_mask = np.fromfile(mask_path, dtype=np.uint8)
    if raw_mask.size == width * height:
        mask = raw_mask.reshape((height, width))
    else:
        m_img = Image.fromarray(raw_mask.reshape((2048, 4096)), mode='L')
        m_img = m_img.resize((width, height), Image.Resampling.BILINEAR)
        mask = np.array(m_img, dtype=np.uint8)
        
    is_land = (mask > 128)
    is_water = ~is_land

    # 2. Process Natural Earth II Master Land Cover & Shaded Relief GeoTIFF
    print(f"Loading Natural Earth II Landcover: {landcover_tif}...")
    lc_img = Image.open(landcover_tif)
    if lc_img.size != (width, height):
        print(f"Resampling from {lc_img.size} to {width}x{height} with Lanczos...")
        lc_img = lc_img.resize((width, height), Image.Resampling.LANCZOS)
    
    lc_arr = np.array(lc_img, dtype=np.float32) / 255.0

    # 3. Apply Grand-Strategy Military Color Grading
    # Convert slightly from bright textbook GIS to moody, matte, command-room strategic atlas
    # Desaturate slightly, deepen shadows, preserve physical relief and natural biomes
    luma = lc_arr[:, :, 0] * 0.299 + lc_arr[:, :, 1] * 0.587 + lc_arr[:, :, 2] * 0.114
    
    # Atmospheric military grading (restrained saturation, deep foliage, warm sand, slate mountains)
    graded_land = np.zeros((height, width, 3), dtype=np.float32)
    
    # Rich matte color balance
    sat_factor = 0.88
    contrast_factor = 1.08
    
    for c in range(3):
        # Saturation adjustment
        ch = luma + (lc_arr[:, :, c] - luma) * sat_factor
        # Gentle S-curve contrast (deepens shadows slightly, retains crisp mountain highlights)
        ch = np.power(np.clip(ch, 0.0, 1.0), contrast_factor)
        graded_land[:, :, c] = ch

    # Convert to 8-bit RGBA Master Relief
    relief_rgba = np.zeros((height, width, 4), dtype=np.uint8)
    for c in range(3):
        relief_rgba[:, :, c] = np.clip(graded_land[:, :, c] * 255.0, 0, 255).astype(np.uint8)
    relief_rgba[:, :, 3] = mask

    print(f"Writing High-Res Master Relief {output_relief_rgba} ({relief_rgba.nbytes} bytes)...")
    relief_rgba.tofile(output_relief_rgba)
    img_relief = Image.fromarray(relief_rgba, mode='RGBA')
    img_relief.save(output_relief_png, optimize=True)

    # 4. Process Natural Earth Ocean Bottom Bathymetry
    print(f"Loading Ocean Bottom Bathymetry: {bathy_tif}...")
    bathy_img = Image.open(bathy_tif)
    if bathy_img.size != (width, height):
        print(f"Resampling bathymetry from {bathy_img.size} to {width}x{height} with Lanczos...")
        bathy_img = bathy_img.resize((width, height), Image.Resampling.LANCZOS)
        
    bathy_arr = np.array(bathy_img, dtype=np.float32)
    if bathy_arr.ndim == 2:
        bathy_luma = bathy_arr / 255.0
    else:
        bathy_luma = (bathy_arr[:, :, 0] * 0.299 + bathy_arr[:, :, 1] * 0.587 + bathy_arr[:, :, 2] * 0.114) / 255.0

    min_b = np.percentile(bathy_luma, 1)
    max_b = np.percentile(bathy_luma, 99)
    norm_bathy = np.clip((bathy_luma - min_b) / (max_b - min_b + 1e-5), 0.0, 1.0)
    
    # Coastal Contact Ambient Occlusion against land
    from scipy.ndimage import distance_transform_edt
    dist_from_land = distance_transform_edt(is_water)
    coastal_ao = np.clip(dist_from_land / 4.0, 0.0, 1.0)

    # Underwater hillshade / ridges
    hillshade = np.clip((bathy_luma - 0.5) * 0.6 + 0.5, 0.2, 0.8)

    bathy_rgba = np.zeros((height, width, 4), dtype=np.uint8)
    bathy_rgba[:, :, 0] = (norm_bathy * 255.0).astype(np.uint8)     # R = Bathymetric Depth (0=abyss, 255=shallow shelf)
    bathy_rgba[:, :, 1] = (coastal_ao * 255.0).astype(np.uint8)     # G = Coastal Contact Shadow
    bathy_rgba[:, :, 2] = (hillshade * 255.0).astype(np.uint8)      # B = Mid-ocean ridge hillshade
    bathy_rgba[:, :, 3] = (is_water * 255).astype(np.uint8)         # A = Water Mask

    print(f"Writing Master Bathymetry {output_bathy_rgba} ({bathy_rgba.nbytes} bytes)...")
    bathy_rgba.tofile(output_bathy_rgba)
    img_bathy = Image.fromarray(bathy_rgba, mode='RGBA')
    img_bathy.save(output_bathy_png, optimize=True)

    print("=== [ART-03] NATURAL EARTH II HIGH-RES MASTER COMPILATION COMPLETE! ===")

if __name__ == '__main__':
    compile_natural_earth_master_hd(width=4096, height=2048)
