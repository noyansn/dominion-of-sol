...
# 4. Clean orphan server.exe or vite processes in this repository
...
Get-CimInstance Win32_Process -Filter "Name = 'server.exe'" -ErrorAction SilentlyContinue | ForEach-Object {
    if ($_.ExecutablePath -and $_.ExecutablePath.ToLower().Contains($repoNorm)) {
        Write-Host "[ORPHAN DETECTED] Terminating orphan server.exe PID $($_.ProcessId)..." -ForegroundColor Yellow
...
$buildOutput = & cargo build --release --bin server --manifest-path $serverManifest 2>&1
...
Write-Host "[BUILD READY] Binary: $CanonicalServerExe ($([math]::Round($binaryInfo.Length / 1MB, 2)) MB, LastModified=$($binaryInfo.LastWriteTime.ToString('o')))" -ForegroundColor Green
...
Write-Host "[SERVER READY] PID=$ServerPid, EmbeddedCommit=$ServerCommit, Protocol=$($serverWelcomeJson.protocolVersion)" -ForegroundColor Green
