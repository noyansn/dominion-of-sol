...
pub fn neighbors8(index: usize) -> [Option<usize>; 8] {
    ...
    let sw = (y + 1 < WORLD_HEIGHT).then(|| (y + 1) * WORLD_WIDTH + (x + WORLD_WIDTH - 1) % WORLD_WIDTH);
    [n, e, s, w, ne, nw, se, sw]
}
...
/// Strictly monotonic with mild diminishing returns: C = 450*N + 4.0*N^2
...
pub fn generate_focus_corridor_patch(
    ...
    let ax = (anchor_idx % WORLD_WIDTH) as f64;
    let ay = (anchor_idx / WORLD_WIDTH) as f64;
    let tx = (target_idx % WORLD_WIDTH) as f64;
    let ty = (target_idx / WORLD_WIDTH) as f64;

    let mut dir_x = wrapped_dx(ax, tx);
    let mut dir_y = ty - ay;
    let mut dir_len = (dir_x * dir_x + dir_y * dir_y).sqrt();

    if dir_len <= 1e-5 {
        let friendly_neighbors: Vec<usize> = legal_land_neighbors(cells, anchor_idx)
            .into_iter()
            .filter(|&adj| cells[adj].owner_id == owner && cells[adj].terrain_type == 0)
            .collect();
        if !friendly_neighbors.is_empty() {
            let mut fx_sum = 0.0;
            let mut fy_sum = 0.0;
            for &f in &friendly_neighbors {
                let fx = (f % WORLD_WIDTH) as f64;
                let fy = (f / WORLD_WIDTH) as f64;
                fx_sum += ax + wrapped_dx(ax, fx);
                fy_sum += fy;
            }
            let avg_fx = fx_sum / friendly_neighbors.len() as f64;
            let avg_fy = fy_sum / friendly_neighbors.len() as f64;
            dir_x = wrapped_dx(avg_fx, ax);
            dir_y = ay - avg_fy;
            dir_len = (dir_x * dir_x + dir_y * dir_y).sqrt();
        }
    }

    let (ux, uy) = if dir_len > 1e-5 {
    ...
    while chosen.len() < requested_size {
        ...
        // 2. Lateral width starts at 2.2 cells and widens smoothly to form a cohesive lobe/wedge
        let max_wedge_width = (3.5 + 0.08 * (requested_size as f64)).min(7.0);
        let allowed_width = (2.2 + 0.25 * proj_forward.max(0.0)).min(max_wedge_width);
        ...
        for n in legal_land_neighbors(cells, idx) {
            if cells[n].terrain_type == 0 && cells[n].owner_id == 0 && !chosen.contains(&n) {
                ...
                let n_allowed_lat = (2.2 + 0.25 * n_proj.max(0.0)).min(max_wedge_width);
                ...
                if queued.insert(n) {
                    ...
                    let cos_theta = if n_v_len > 1e-4 { (n_proj / n_v_len).clamp(-1.0, 1.0) } else { 1.0 };
                    ...
                    let irregularity = (((n as u32).wrapping_mul(2_654_435_761) ^ (owner as u32).wrapping_mul(805_306_457)) % 37) as i32;

                    let score = 10_000 + support_bonus + alignment_bonus + forward_progress + irregularity
                        - depth_cost
                        - lateral_penalty;
    ...
    let ax = (anchor as usize % WORLD_WIDTH) as f64;
    let ay = (anchor as usize / WORLD_WIDTH) as f64;
    result_cells.sort_by(|&c1, &c2| {
        if c1 == anchor { return std::cmp::Ordering::Less; }
        if c2 == anchor { return std::cmp::Ordering::Greater; }
        let c1x = (c1 as usize % WORLD_WIDTH) as f64;
        let c1y = (c1 as usize / WORLD_WIDTH) as f64;
        let c2x = (c2 as usize % WORLD_WIDTH) as f64;
        let c2y = (c2 as usize / WORLD_WIDTH) as f64;
        let d1 = wrapped_dx(ax, c1x).powi(2) + (c1y - ay).powi(2);
        let d2 = wrapped_dx(ax, c2x).powi(2) + (c2y - ay).powi(2);
...
pub fn generate_frontier_distribution_patch_targeted(
    ...
    for &c in &result_cells {
        if legal_land_neighbors(cells, c as usize).into_iter().any(|n| cells[n].owner_id == owner) {
            depth.insert(c, 0);
            q.push_back(c);
        }
    }
